...
const Dashboard = () => {
  ...
  const [selectedTechnicalDetails, setSelectedTechnicalDetails] = useState<
    any[]
  >([]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const uniqueCustomerNames = [
        ...new Set(
          (inquiries as { customerName: string }[]).map(
      ...
      const uniqueCustomerTypes = [
        ...new Set(
          (inquiries as { customerType: string }[]).map(
  ...
  const updateChartData = (data: any[]) => {
    ...
    for (const inquiry of data) {
      ...
      customerTypeCounts[customerType] =
        (customerTypeCounts[customerType] || 0) + 1;
    }
    ...
    setCustomerTypeData(
      Object.entries(customerTypeCounts).map(([name, value]) => ({
        name,
        value,
      }))
    );
  ...
  const applyFilters = () => {
    ...
    if (offerStatusFilter)
      filtered = filtered.filter((r) => r.offerStatus === offerStatusFilter);
    if (regionFilter)
      filtered = filtered.filter((r) => r.region === regionFilter);
    if (customerNameFilter)
      filtered = filtered.filter((r) => r.customerName === customerNameFilter);
    if (customerTypeFilter)
      filtered = filtered.filter((r) => r.customerType === customerTypeFilter);
    if (fromDateFilter)
      filtered = filtered.filter(
        (item) => new Date(item.enquiryDate) >= new Date(fromDateFilter)
      );
    if (toDateFilter)
      filtered = filtered.filter(
        (item) => new Date(item.enquiryDate) <= new Date(toDateFilter)
      );
  ...
  const updateMonthlyStatusData = (data: any[]) => {
    ...
    data.forEach((inquiry) => {
      ...
      if (!isNaN(date.getTime())) {
        ...
        const status = inquiry.status || "Unknown";
        monthlyMap[monthYear][status] =
          (monthlyMap[monthYear][status] || 0) + 1;

        const packageAmount = parseFloat(inquiry.totalPackage) || 0;
        // 💰 Sum `totalPackage` grouped by offerStatus
        monthlyMap[monthYear][status] =
          (monthlyMap[monthYear][status] || 0) + packageAmount;
      }
    });
    ...
    const result = Object.entries(monthlyMap).map(([monthYear, statusSums]) => {
      const total = Object.values(statusSums).reduce(
        (sum, val) => sum + val,
        0
      );
      return {
  ...
  const exportToExcel = () => {
    ...
    saveAs(
      dataBlob,
      `Inquiry_Dashboard_${new Date().toISOString().slice(0, 10)}.xlsx`
    );
  };
