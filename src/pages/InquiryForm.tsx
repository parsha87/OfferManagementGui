...
const InquiryForm = () => {
  ...
  const [motorMappingData, setMotorMappingData] = useState<MotorMapping | null>(
    null
  );
  ...
  const [voltages, setVoltages] = useState<
    { label: string; value: number; inputValue?: string }[]
  >([]);
  const [startsPerHour, setStartsPerHour] = useState<
    { label: string; value: number; inputValue?: string }[]
  >([]);
  ...
  const [accessories, setAccessories] = useState<
    { label: string; value: number; inputValue?: string }[]
  >([]);
  const [nonStandardFeatures, setNonStandardFeatures] = useState<
    { label: string; value: number; inputValue?: string }[]
  >([]);
  ...
  const [segments, setSegments] = useState<
    { label: string; value: number; inputValue?: string }[]
  >([]);
  const [stdPaymentTerms, setStdPaymentTerms] = useState<
    { label: string; value: number; inputValue?: string }[]
  >([]);
  ...
  const handleBrandChange = (e: any) => {
    ...
    // Format value to 2 decimal places for 'kw' and 'hp'
    let newValue = value;
    if (name === "kw" || name === "hp") {
      newValue = value
        .replace(/[^\d.]/g, "")
    ...
    let updated = { ...brandInput, [name]: newValue };

    // Mark amount as manually edited when 'amount' changes
    if (name === "amount") {
      updated.isAmountManuallyEdited = true;
    }

    // Calculate 'amount' if isListPrice is true and price or quantity changed,
    // and amount was not manually edited
    if (
      // check current object flag
      ...
      const price =
        parseFloat(
          name === "techDetailsListPrice"
            ? newValue
            : updated.techDetailsListPrice
        ) || 0;
      const quantity =
        parseFloat(name === "quantity" ? newValue : updated.quantity) || 0;

      updated.amount = (price * quantity).toFixed(2);
    }

    // Special field resets
    if (name === "safeAreaHazardousArea" && newValue !== "Hazardous Area") {
      updated.zone = "";
      updated.gasGroup = "";
    }
    if (name === "encoderMountingIfYes" && newValue !== "Yes") {
      updated.zone = "";
      updated.gasGroup = "";
    }
    ...
    if (editIndex !== null) {
      ...
      setFormData((prevData: any) => {
        const updatedBrandMapping = [
          ...(prevData.technicaldetailsmappings || []),
          { ...brandInput },
        ];
  ...
  const handleEditBrand = (index: any) => {
    ...
    // selected.islistprice = Boolean(selected.islistprice); // Format amount to 2 decimal places
  ...
  const handleDeleteBrand = () => {
    setFormData((prev) => ({
      ...prev,
      technicaldetailsmappings: prev.technicaldetailsmappings.filter(
        (_, i) => i !== deleteIndex
      ),
    }));
  ...
  const validateFormData = (): boolean => {
    ...
    // Contact Info
    if (!formData.custPhoneNo)
      return toastError("Customer Phone No is required");
    ...
    if (!/\S+@\S+\.\S+/.test(formData.custEmail))
      return toastError("Email is invalid");

    // Address
    if (!formData.custAddress)
      return toastError("Customer Address is required");
    ...
    if (!formData.customerRfqno)
      return toastError("Customer RFQ No is required");
    if (!formData.customerRfqdate)
      return toastError("Customer RFQ Date is required");
    ...
    if (formData.totalPackage < 0)
      return toastError("Total Package cannot be negative");
  ...
  const handleEnumChange = async (type: ListOfValueType, value: string) => {
    ...
    const isExist = listOfValues.some(
      (c) => c.value.toLowerCase() === value.toLowerCase() && c.type === type
    );
    if (!isExist) {
      try {
  ...
  const handleDeleteFile = (file: any) => {
    api
      .delete(`Inquiry/deleteFile/${file.attachmentId}`)
      .then((res) => {
        ...
        setFormData((prevData) => ({
          ...
          uploadedFiles: prevData.uploadedFiles.filter(
            (uploadedFile) => uploadedFile.attachmentId !== file.attachmentId
          ),
        }));
      })
      .catch((err) => {
  ...
  const handleDownloadFile = (file: any) => {
    ...
    api
      .get(`Inquiry/getFileById/${file.attachmentId}`, { responseType: "blob" }) // Set responseType to 'blob' for file download
      .then((res) => {
  ...
  const handleCustomerChange = async (newValue: any) => {
    ...
    if (newValue.value && newValue.value > 0) {
      const res1 = await api.get<any>(`Customer/${newValue.value}`);

      if (!res1.data) {
        toast.error("No customer found with the given ID.");
        return;
      } else {
        customer = res1.data;
        // Use `customer` from here
      }
    }
    ...
    if (customer) {
      setFormData((prev) => ({
        ...prev,
        cpfirstName: customer.firstname,
        cplastName: customer.lastname,
      }));
    }
  };
    ...
    <Box sx={{ p: 0 }}>
              ...
              <TextField
                ...
                value={
                  formData.customerRfqdate
                    ? new Date(formData.customerRfqdate)
                        .toISOString()
                        .split("T")[0]
                    : ""
                }
              ...
              <TextField
                ...
                value={
                  formData.offerDueDate
                    ? new Date(formData.offerDueDate)
                        .toISOString()
                        .split("T")[0]
                    : ""
                }
          ...
          <Grid container spacing={2}>
            ...
            <Grid size={{ xs: 12, sm: 3 }}>
              <Autocomplete
                ...
                value={formData.region}
                onChange={handleEnumChangeValue(
                  ListOfValueType.Region,
                  setFormData
                )}
                filterOptions={(
                  options,
          ...
          <Grid container spacing={2}>
            ...
            <Grid size={{ xs: 12 }}>
              ...
              <Box sx={{ overflowX: "auto", width: 1580 }}>
                {formData.technicaldetailsmappings &&
                  formData.technicaldetailsmappings.length > 0 && (
                    <TableContainer
                      sx={{
                        maxHeight: 300,
                    ...
                    >
                      <Table stickyHeader size="small">
                        ...
                        <TableBody>
                          {formData.technicaldetailsmappings.map(
                            (brand, index) => (
                              <TableRow key={index}>
                                <TableCell>
                                  <Button
      ...
      >
        ...
        <DialogActions>
          ...
          <Button onClick={handleSubmitVistDes} variant="contained">
            Submit
          </Button>
        </DialogActions>
      </Dialog>
        ...
        <DialogContent dividers>
          <Grid container spacing={2} sx={{ mt: 1, mb: 2 }}>
            ...
            <Grid container spacing={2}>
              ...
              {brandInput.duty !== "S1" &&
                brandInput.duty !== "S3" &&
                brandInput.duty !== "S2-15min" &&
                brandInput.duty !== "S2-30min" && (
                  <Grid size={{ xs: 12, sm: 4 }}>
                    <Autocomplete
                      freeSolo
              ...
              <Grid size={{ xs: 12, sm: 4 }}>
                <Autocomplete
                  ...
                  onChange={handleMultiEnumChangeValue(
                    ListOfValueType.Accessories,
                    setBrandInput
                  )}
              ...
              <Grid size={{ xs: 12, sm: 4 }}>
                <Autocomplete
                  ...
                  onChange={handleEnumChangeValue(
                    ListOfValueType.NonStandardFeatures,
                    setBrandInput
                  )}
                  filterOptions={(
                    options,
              ...
              <Grid size={{ xs: 12, sm: 1 }}>
                <FormControlLabel
                  control={
                    <Switch
                      checked={brandInput.islistprice}
                      onChange={(e) =>
                        setBrandInput((prev) => ({
                          ...prev,
                          islistprice: e.target.checked,
                        }))
                      }
                      color="primary"
                    />
      ...
      <Dialog open={open} onClose={() => setOpen(false)}>
        ...
        <DialogActions>
          ...
          <Button onClick={handleSubmitCust} variant="contained">
            Add
          </Button>
        </DialogActions>
      </Dialog>
